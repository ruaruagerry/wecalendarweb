...
// WriteContext send context to peer
func (ctx *StupidContext) WriteContext(buf []byte) {

	if buf != nil {
		http.ServeContent(ctx.W, ctx.r, ctx.r.URL.Path, time.Now(), bytes.NewReader(buf))
	} else {
...
// GetHTTPRequest 获取请求结构体
func (ctx *StupidContext) GetHTTPRequest() *http.Request {
	return ctx.r
}
